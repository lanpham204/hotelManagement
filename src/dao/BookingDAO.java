...
public class BookingDAO extends HotelDAO<Booking, Integer> {
    String INSERT_SQL = "INSERT INTO PhieuDatPhong(MaKH, MaPhong, NgayGioNhan, NgayGioTra, TongTien) VALUES (?, ?, ?, ?, ?)";
    String UPDATE_SQL = "UPDATE PhieuDatPhong SET MaKH = ?, MaPhong = ?, NgayGioNhan = ?, NgayGioTra = ?, TongTien = ? WHERE MaPDP = ?";
    ...
    public void insert(Booking entity) {
        try {
            jdbcHelper.update(INSERT_SQL, entity.getIdGuest(), entity.getIdRoom(), entity.getStartDate(),
                    entity.getEndDate(), entity.getTotalMoney());
    ...
    public void update(Booking entity) {
        try {
            jdbcHelper.update(UPDATE_SQL, entity.getIdGuest(), entity.getIdRoom(), entity.getStartDate(),
                    entity.getEndDate(), entity.getTotalMoney(), entity.getId());
        } catch (SQLException ex) {
            Logger.getLogger(BookingDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    protected List<Booking> selectBySql(String sql, Object... args) {
        ...
        try (ResultSet resultSet = jdbcHelper.query(sql, args)) {
            while (resultSet.next()) {
                ...
                booking.setStartDate(resultSet.getDate("NgayGioNhan"));
                booking.setEndDate(resultSet.getDate("NgayGioTra"));
